...
/*=============== Lesson learn =======================
| 1. PROCEDUR akan berparameter passing by reference |
|  ( *D ) di method dan ( &D ) di main.              |
| 2. FUNCTION akan berparamter passing by value      |
|  ( D ) di method dan ( D1/Nama variabel globalnya) |
/*====================================================


/*********** Operasi Primitif ************/
void CreateDate (date * D) 
/* Constructor atau Creator Membentuk sebuah DATE, dengan nilai default adalah 01/01/1900 */
{
		SetTgl ((* D).Tgl, &(* D)); // &(* D)) : Untuk mengkonsistenkan D
		SetBln ((* D).Bln, &(* D)); //           pada parameter maka tetap 
		SetThn ((* D).Thn, &(* D)); //          menggunakan *D 
}
...
/* Memberikan tanggal terakhir dari sebuah bulan */
{
	...
	if((D.Bln % 2 == 0) && (D.Bln != 2) && (D.Bln != 12)){ //Bulan Genap kecuali februari dan Desember
		TAkhir = 30;	
	} else if((D.Bln % 2 == 1) || (D.Bln == 12)){ //Bulan Ganjil termasuk Desember
		TAkhir = 31;
	...
	Asumsi : 1 tahun = 365 hari */
{
	...
	unsigned int SelisihHari;
	if(isKabisat(D1) == true){ //Pengecekan selisih ketika tahun kabisat
		if(GetBln(D1) == GetBln(D2)) { // Dibulan yang sama
			SelisihHari = ((GetThn(D2) - GetThn(D1)) * 365) + ((GetBln(D2) - GetBln(D1)) * 29) + (GetTgl(D2) - GetTgl(D1));
		} else if(GetBln(D1) == 2 || GetBln(D1) < 2) { // Pada bulan februari atau Januari
			SelisihHari = ((GetThn(D2) - GetThn(D1)) * 365) + ((GetBln(D2) - GetBln(D1)) * 29) + (GetTgl(D2) - GetTgl(D1) );
		} else {
			SelisihHari = ((GetThn(D2) - GetThn(D1)) * 365) + ((GetBln(D2) - GetBln(D1)) * 29) + (GetTgl(D2) - GetTgl(D1));
		}	
	} else if ((GetBln(D1) % 2 == 0) && (GetBln(D2) % 2 == 0)) {//Bulan genap
			SelisihHari = ((GetThn(D2) - GetThn(D1)) * 365) + ((GetBln(D2) - GetBln(D1)) * 30 ) + (GetTgl(D2) - GetTgl(D1));

		}
	} else{
		SelisihHari = ((GetThn(D2) - GetThn(D1)) * 365) + ((GetBln(D2) - GetBln(D1)) *  + (GetTgl(D2) - GetTgl(D1));
	}
	
	
	
	printf(" \n\n\n\n\n\n\t\t\t\tSelisih hari dari tanggal %d/%d/%d <-----> " , GetTgl(D1) , GetBln(D1) , GetThn(D1));
